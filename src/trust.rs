...
//! already consumes; this crate is its third reader, not a new format.
//!
//! # The loop this closes
//!
//! ```text
//! check --signer ci@payments --audit-out log.json
...
//!   → lex producer-trust keyring --min-trust 700 --out trusted.json
//!   → check --signer ci@payments --trusted-keys trusted.json
//! ```
//!
...
//! standing does is decide whether a **wildcard** in the grant covers
//! this submitter's mutating changes, or whether each verb has to be
//! named. An unknown submitter is held to the narrower reading of the
//! same grant — never to a wider one. If a score could ever admit an
//! effect the manifest does not, that would be a second source of
//! authority, which is the one thing this project forbids.
...
///
/// A CI pipeline identity, an agent key — whatever the operator keys
/// their record under. The gate records it and, when a keyring was
/// supplied, consults it.
...
mod tests {
    ...
    fn a_keyring_reads_the_shape_lex_lang_writes() {
        let k = Keyring::from_json(r#"{"trusted":["ci@payments","agent-7"]}"#).unwrap();
        assert!(k.admits("ci@payments"));
        assert!(!k.admits("ci@billing"));
    }
    ...
    fn an_empty_keyring_trusts_nobody() {
        ...
        assert_eq!(k.standing_of("ci@payments"), Standing::Unknown);
    }
    ...
    fn standing_comes_from_the_keyring() {
        let k = Keyring::new(["ci@payments"]);
        assert_eq!(
            Submitter::against("ci@payments", &k).standing,
            Standing::Trusted
        );
        assert_eq!(
            Submitter::against("ci@billing", &k).standing,
            Standing::Unknown
        );
        assert_eq!(
            Submitter::unconsulted("ci@payments").standing,
